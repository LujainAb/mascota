...
def create_app(test_config=None):
    ...
    def get_shelters(jwt):
        ...
        # checkign if the returned result from the query is empty or not
        # if yes return an appropriate error
    ...
    def shelter_search(jwt):
        ...
        shelters_query = Shelter.query.filter(Shelter.name.ilike('%{}%'.format(searchTerm)))  # noqa
    ...
    def get_pets(jwt):
        ...
        # checkign if the returned result from the query is empty or not , if yes return an appropriate error  # noqa
        if len(pets_query) == 0:
            abort(404)
    ...
    def pet_search(jwt):
        ...
        pets_query = Pet.query.filter(Pet.type.ilike('%{}%'.format(searchTerm)))  # noqa
    ...
    def add_pet(jwt):
        ...
        if(pet_name is None or pet_type is None or pet_sex is None or pet_age is None):  # noqa
            abort(400)

        pet = Pet(name=pet_name, type=pet_type, breed=pet_breed, sex=pet_sex, age=pet_age, behaviour=pet_behaviour)  # noqa
